...
# Replace apex import with torch.cuda.amp
# No explicit import needed - torch.cuda.amp is built-in
...
def make_optimizer_and_schedule(args, model, checkpoint, params):
    ...
    *Internal Function* (called directly from train_model)
    ...
    if checkpoint:
        ...
        if 'scaler' in checkpoint and checkpoint['scaler'] not in [None, 'N/A'] and scaler is not None:
...
def train_model(args, model, loaders, *, checkpoint=None, dp_device_ids=None,
    ...
    if checkpoint:
        ...
        best_prec1 = checkpoint[prec1_key] if prec1_key in checkpoint \
            else _model_loop(args, 'val', val_loader, model, None, start_epoch-1, args.adv_train, writer=None, scaler=scaler)[0]
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_prec1, train_loss = _model_loop(args, 'train', train_loader, 
                model, opt, epoch, args.adv_train, writer, scaler)
        ...
        sd_info = {
            ...
            'epoch': epoch+1,
            'scaler': scaler.state_dict() if args.mixed_precision and scaler is not None else None,
        }
        ...
        if should_log or last_epoch or should_save_ckpt:
            ...
            with ctx:
                prec1, nat_loss = _model_loop(args, 'val', val_loader, model, 
                        None, epoch, False, writer, scaler)
            ...
            adv_val = should_adv_eval and _model_loop(args, 'val', val_loader,
                    model, None, epoch, True, writer, scaler)
...
def _model_loop(args, loop_type, loader, model, opt, epoch, adv, writer, scaler):
    """
    ...
    for i, (inp, target) in iterator:
       # measure data loading time
        ...
        if is_train:
            opt.zero_grad()
            if args.mixed_precision and scaler is not None:
                # Use GradScaler for backward and optimizer step
